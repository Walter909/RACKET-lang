...
(define-type ExprC (U numC binop leq0? idC AppC))
...
(struct AppC ([fname : Symbol] [paramVals : (Listof ExprC)])#:transparent)
...
(define (subst-one [what : ExprC] [for : Symbol] [in : ExprC]) : ExprC
  (match in
    ...
    [(idC s) (cond
               [(symbol=? s for) what]
               [else in])]
    [(AppC f vals) (AppC f (map (lambda ([val : ExprC])
...
(define (interp [exp : ExprC] [lstfuncd : (Listof FundefC)]) : Real
  (match exp
    ...
    [(AppC (? symbol? f) (list args ...)) (interpret-funcalls f (my-params args) lstfuncd)]))
  ...
  (match (expected-type s)
    ...
    ['AppC (match s
                 [(list (? symbol? a) b ...) (cond
                                               [(equal? (length b) 0) (AppC a (list))]
                                               [else (AppC a (map parse b))])]
...
(check-equal? (parse-prog '{{fn {f x} {+ x 14}} {fn {main} {f 14}}})
              (list (FundefC 'f (list 'x) (binop '+ (idC 'x) (numC 14)))
                    (FundefC 'main (list) (AppC 'f (list (numC 14))))))
...
(check-equal? (interp (AppC 'f (list (numC 3)))
...
(check-equal? (parse '{^ 3 4}) (AppC '^ (list (numC 3) (numC 4))))
...
(check-equal? (subst-one (numC 3) 'x (AppC 'f (list (idC 'x)))) (AppC 'f (list (numC 3))))
(check-equal? (subst-one (numC 3) 'x (AppC 'f (list (idC 'y)))) (AppC 'f (list (idC 'y))))
...
(check-equal? (subst-many (list (numC 3) (numC 5)) (list 'x 'y)
                          (AppC 'f (list (idC 'x) (idC 'y)))) (AppC 'f (list (numC 3) (numC 5))))
